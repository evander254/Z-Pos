  ...
  useEffect(() => {
    ...
    async function redirectToWorkspace() {
      ...
      if (typeof window !== "undefined") {
        const storedEmployee = localStorage.getItem("zpos-mock-employee");
        if (storedEmployee) {
          try {
            const employee = JSON.parse(storedEmployee) as { business_id?: string };
            ...
            console.warn("Failed to resolve stored employee session", error);
          }
        }
      }
      ...
      if (!slug) {
        const { data } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        const business = (data as { businesses?: { slug?: string } | null } | null)?.businesses;
        slug = business?.slug ?? null;
      }

      if (cancelled) return;
      if (slug) nav({ to: `/t/${slug}` });
      else nav({ to: "/onboarding" });
    }

    redirectToWorkspace().catch((error) => {
      console.warn("Failed to redirect active session", error);
    ...
    <header className="sticky top-0 z-50 backdrop-blur-md bg-white/70 border-b border-slate-100 dark:bg-zinc-950/70 dark:border-zinc-800 transition-colors duration-300">
      <div className="mx-auto max-w-7xl px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
          ...
          <a href="#businesses" className="hover:text-foreground">
            Industries
          </a>
      ...
      <div className="mx-auto max-w-7xl px-6 pt-20 pb-28 text-center">
        ...
        >
          ...
          <p className="mt-6 max-w-2xl mx-auto text-lg text-muted-foreground">
            One platform for supermarkets, restaurants, pharmacies, salons and more. Choose your
            business type — get a workspace tailored to you in seconds.
          </p>
          <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <Link to="/auth/signup">
              <Button
                size="lg"
                className="gradient-violet text-white border-0 hover:opacity-90 h-12 px-6"
              >
            ...
            <div className="rounded-2xl bg-white/50 dark:bg-zinc-950/50 backdrop-blur-md p-6 md:p-10 border border-slate-200/50 dark:border-zinc-800/50 overflow-hidden relative">
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="text-left">
                  <div className="text-xl font-bold">
                    {BUSINESS_TYPE_MAP[activeIndustry]?.label || "Dashboard"} Workspace
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {BUSINESS_TYPE_MAP[activeIndustry]?.tagline}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
                {[
                  ...
                  <div
                    key={k.label}
                    className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md rounded-xl p-5 text-left border border-slate-100 dark:border-zinc-800 shadow-sm transition-all hover:shadow-md"
                  >
    ...
    <div className="border-y border-border/40 py-8">
      <div className="mx-auto max-w-7xl px-6 flex flex-wrap items-center justify-center gap-x-12 gap-y-4 text-muted-foreground text-sm">
        ...
        {["Nairobi", "Mombasa", "Kisumu", "Eldoret", "Nakuru", "Thika"].map((c) => (
          <span key={c} className="font-medium tracking-wide opacity-80">
            {c}
          </span>
        ))}
      </div>
...
function BusinessGrid({
  ...
  activeIndustry: BusinessTypeKey;
  setActiveIndustry: (i: BusinessTypeKey) => void;
}) {
  return (
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-3 text-muted-foreground">
            Pick your business type during signup and we'll set up everything you need.
          </p>
        </div>
          ...
          {BUSINESS_TYPES.map((b, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Overlay Gradient */}
                <div
                  className={`absolute inset-0 z-10 pointer-events-none transition-colors duration-300 ${isActive ? "bg-gradient-to-t from-violet-950/95 via-slate-900/60 to-transparent" : "bg-gradient-to-t from-slate-900/90 via-slate-900/50 to-transparent"}`}
                />

                {/* Content */}
                <div className="relative z-10 flex flex-col h-full justify-between items-start w-full">
                  <div
                    className={`h-10 w-10 rounded-lg backdrop-blur-md border flex items-center justify-center transition-colors duration-300 ${isActive ? "bg-violet-500/20 border-violet-400/50" : "bg-white/10 border-white/20"}`}
                  >
                    <b.icon
                      className={`h-5 w-5 transition-colors ${isActive ? "text-violet-200" : "text-white"}`}
                    />
                  </div>
                  <div className="mt-4 w-full">
                    <div
                      className={`font-bold text-base leading-snug transition-colors ${isActive ? "text-white" : "text-slate-100"}`}
                    >
                      {b.label}
                    </div>
  ...
  const items = [
    {
      icon: ScanBarcode,
      t: "Smart Checkout",
      d: "Barcode + camera scanning, holds, refunds, discounts and printable receipts.",
      color: "text-blue-500",
      bg: "bg-blue-500/10",
    },
    {
      icon: CreditCard,
      t: "M-Pesa, Card & Cash",
      d: "Accept every payment your customers use. Reconcile instantly.",
      color: "text-emerald-500",
    ...
    },
    {
      ...
      d: "Sales heatmaps, best sellers, profit estimates and employee performance.",
    ...
    },
    {
      icon: Smartphone,
      t: "Mobile-first POS",
      d: "Run your shop from any phone, tablet or touchscreen device.",
      color: "text-amber-500",
      bg: "bg-amber-500/10",
    },
    {
      icon: ShieldCheck,
      t: "Tenant-isolated",
      d: "Row-level security and per-business workspaces keep your data yours.",
      color: "text-rose-500",
      bg: "bg-rose-500/10",
    },
    {
      icon: Zap,
      t: "AI assistant",
      d: "Predict low stock, get smart sales insights and business recommendations.",
    ...
    <section id="features" className="py-24 grid-bg">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-3 text-muted-foreground">
            A premium POS plus inventory, employees, customers and analytics — all in one place.
          </p>
        </div>
            ...
            <div key={f.t} className="group perspective-1000 h-[200px]">
              <div className="relative w-full h-full transition-transform duration-700 transform-style-3d flip-card-inner">
                ...
                <div className="absolute inset-0 backface-hidden glass rounded-2xl p-6 flex flex-col items-center justify-center text-center shadow-sm">
                  <div
                    className={`h-16 w-16 rounded-full flex items-center justify-center ${f.bg} mb-4`}
                  >
                ...
                <div className="absolute inset-0 backface-hidden rotate-y-180 glass rounded-2xl p-6 flex flex-col justify-center border-violet-200/50 dark:border-violet-500/30 shadow-xl bg-violet-50/80 dark:bg-violet-950/40 backdrop-blur-md">
                  <div className="font-semibold text-lg mb-2 text-violet-700 dark:text-violet-400">
                    {f.t}
                  </div>
      ...
      <div className="mx-auto max-w-7xl px-6 grid md:grid-cols-2 gap-12 items-center">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold">
            A POS that feels like an app, not a spreadsheet.
          </h2>
          <p className="mt-4 text-muted-foreground text-lg">
            Touchscreen-optimised checkout, gorgeous dashboards and dark mode by default. Designed
            to look great on the till, on the manager's tablet and on the owner's phone.
          </p>
            ...
            {[
              ...
              <li key={t} className="flex items-center gap-3">
                <div className="flex-shrink-0 h-6 w-6 rounded-full bg-violet-100 dark:bg-violet-500/20 flex items-center justify-center">
                  <Check
                    className="h-3.5 w-3.5 text-violet-600 dark:text-violet-400 font-bold"
                    strokeWidth={3}
                  />
                </div>
        ...
        <div className="glass rounded-3xl p-2 glow-violet shadow-2xl relative perspective-[1000px]">
          <div className="rounded-2xl bg-slate-50 dark:bg-zinc-950 p-6 border border-border/60 overflow-hidden transform-style-3d hover:rotate-y-[-2deg] hover:rotate-x-[2deg] transition-transform duration-500">
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="px-2 py-1 bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400 text-xs font-bold rounded">
                TILL-04
              </div>
            </div>
              ...
              {[
                {
                  name: "Avocado",
                  price: "45",
                  img: "https://images.unsplash.com/photo-1523049673857-eb18f1d7b578?auto=format&fit=crop&q=80&w=200",
                  stock: 12,
                },
                {
                  name: "Coffee Beans",
                ...
                },
                {
                  name: "Milk 1L",
                  price: "120",
                  img: "https://images.unsplash.com/photo-1550583724-b2692b85b150?auto=format&fit=crop&q=80&w=200",
                  stock: 24,
                },
                {
                  name: "Artisan Bread",
                ...
                },
                {
                  name: "Honey 500g",
                  price: "600",
                  img: "https://images.unsplash.com/photo-1587049352847-4d4b12736b51?auto=format&fit=crop&q=80&w=200",
                  stock: 2,
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className="aspect-square rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 flex flex-col p-2 hover:border-violet-400 cursor-pointer transition-colors shadow-sm relative overflow-hidden group"
                >
                  <div className="absolute top-1 right-1 bg-white/90 dark:bg-black/90 backdrop-blur-sm text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm z-10">
                    {item.stock} left
                  </div>
                  ...
                  <div className="text-[10px] sm:text-xs font-medium leading-tight truncate mt-auto">
                    {item.name}
                  </div>
                  <div className="text-[10px] sm:text-xs font-bold text-violet-600 dark:text-violet-400">
                    KES {item.price}
                  </div>
                </div>
              ))}
...
const PLANS = [
  {
    ...
    price: "KES 0",
    per: "/14-day trial",
    desc: "Try every feature, no card needed.",
    features: ["1 business", "Up to 100 products", "1 cashier", "Email support"],
    cta: "Get started",
  },
  {
    name: "Business",
    price: "KES 1,999",
    per: "/month",
    desc: "For growing shops & restaurants.",
    features: [
      "Unlimited products",
      "5 staff",
      "M-Pesa integration",
  ...
  },
  {
    ...
    price: "Custom",
    per: "",
    desc: "Multi-branch & custom integrations.",
    features: ["Unlimited staff", "Multi-branch", "Custom reports", "Dedicated CSM"],
    cta: "Contact sales",
  },
];
...
function Pricing() {
  return (
    <section id="pricing" className="py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center max-w-2xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold">Shop owner packages</h2>
          <p className="mt-3 text-muted-foreground">
            Start with a free trial, upgrade monthly, or choose a custom enterprise setup.
          </p>
        </div>
        <div className="mt-12 grid md:grid-cols-3 gap-5 md:gap-8 items-center max-w-5xl mx-auto">
          {PLANS.map((p) => (
            <div
              key={p.name}
              className={`relative rounded-2xl p-8 transition-transform ${p.featured ? "glass md:scale-105 border-violet-500/50 shadow-[0_0_30px_rgba(139,92,246,0.15)] bg-gradient-to-b from-violet-500/5 to-transparent z-10" : "glass"}`}
            >
              {p.badge && (
                ...
                {p.features.map((f) => (
                  <li key={f} className="flex items-center gap-3">
                    <div className="flex-shrink-0 h-5 w-5 rounded-full bg-violet-100 dark:bg-violet-500/20 flex items-center justify-center">
                      <Check
                        className="h-3 w-3 text-violet-600 dark:text-violet-400 font-bold"
                        strokeWidth={3}
                      />
                    </div>
              ...
              <Link to="/auth/signup">
                <Button
                  className={`mt-8 w-full h-12 font-semibold ${p.featured ? "gradient-violet text-white border-0 shadow-lg hover:opacity-90 hover:shadow-xl transition-all" : "border-2"}`}
...
function FAQ() {
  const qs = [
    {
      q: "Does ZPos work offline?",
      a: "Yes — checkout queues locally and syncs when you're back online.",
    },
    {
      q: "Can I accept M-Pesa?",
      a: "Yes, M-Pesa is built in and reconciles with your daily sales automatically.",
    },
    {
      q: "Do I need a barcode scanner?",
      a: "No — use your phone camera, or plug in any USB scanner.",
    },
    {
      q: "Can I switch business type later?",
      a: "Yes, you can upgrade modules from your dashboard settings any time.",
    },
  ];
  return (
      ...
      <div className="mx-auto max-w-5xl px-6">
        <div className="glass glow-violet rounded-3xl p-12 text-center">
          ...
          <Link to="/auth/signup">
            <Button
              size="lg"
              className="mt-6 gradient-violet text-white border-0 hover:opacity-90 h-12 px-8"
            >
